...
local function loadLibrary(retry)
    retry = retry or 1
    local url = "https://raw.githubusercontent.com/KING-STUDIO-OFFICIAL/KOD-STUDIO/refs/heads/main/UNIVERSAL-library.lua"
    local ok, data = pcall(game.HttpGet, game, url)
    if not ok then
        warn("[KOD HUB] Failed to download library: " .. tostring(data))
        if retry < 3 then
            task.wait(0.5)
            return loadLibrary(retry + 1)
        end
        return false
    end
    local fn, err = loadstring(data)
    if not fn then
        warn("[KOD HUB] Failed to compile library: " .. tostring(err))
        if retry < 3 then
            task.wait(0.5)
            return loadLibrary(retry + 1)
        end
    ...
    if type(lib) ~= "table" then
        warn("[KOD HUB] Library did not return a table")
        if retry < 3 then
            task.wait(0.5)
            return loadLibrary(retry + 1)
        end
    ...
    KODlib = lib
    Environment.KODlib = KODlib
    
    if KODlib._closeNotification == nil then
        KODlib._closeNotification = function() end
    end
...
if not loadLibrary() then
    warn("[KOD HUB] KOD library could not be loaded after retries.")
    error("KOD library could not be loaded. Script cannot continue.")
end
...
if not KODlib or type(KODlib.MakeWindow) ~= "function" then
    error("[KOD HUB] KOD library is not properly loaded.")
end

local Window = KODlib:MakeWindow({"KOD HUB | The Lost Front", "KOD HUB", "LostFrontESP"})

local function hexToColor3(hex)
    local cleanedHex = hex:gsub("#", "")
    if #cleanedHex == 6 then
        local r = tonumber(cleanedHex:sub(1,2), 16) or 0
        local g = tonumber(cleanedHex:sub(3,4), 16) or 0
        local b = tonumber(cleanedHex:sub(5,6), 16) or 0
...
local Tabs = {}
Tabs.SilentAim = Window:MakeTab({"Silent Aim", "crosshair"})
Tabs.ESP = Window:MakeTab({"ESP", "eye"})
Tabs.Info = Window:MakeTab({"Info", "info"})

pcall(function()
    Tabs.Info:AddParagraph({"KOD HUB", "Información General"})
    Tabs.Info:AddParagraph({"GAME TITLE", "The Lost Front"})
    Tabs.Info:AddParagraph({"BY KING", "Creador Oficial"})
    Tabs.Info:AddParagraph({"DISCORD", "https://discord.gg/k56jB8CFhW"})
    Tabs.Info:AddParagraph({"Perfil ID", "134118410117592"})
    Tabs.Info:AddParagraph({"Cover ID", "95103720591784"})
end)

Tabs.SilentAim:AddSection("Silent Aim")
Tabs.SilentAim:AddToggle({"Enabled", Controller.Settings.SilentAimEnabled, function(v) Controller.Settings.SilentAimEnabled = v if not v then Controller.SilentTarget = nil Controller.SilentTargetKind = nil end end, "SilentAimEnabled", "Enable or disable silent aim"})
Tabs.SilentAim:AddToggle({"Team check", Controller.Settings.SilentAimTeamCheck, function(v) Controller.Settings.SilentAimTeamCheck = v end, "SilentAimTeamCheck"})
Tabs.SilentAim:AddToggle({"Visibility check", Controller.Settings.SilentAimVisibilityCheck, function(v) Controller.Settings.SilentAimVisibilityCheck = v end, "SilentAimVisibilityCheck"})
Tabs.SilentAim:AddToggle({"Movement and drop prediction", Controller.Settings.SilentAimPrediction, function(v) Controller.Settings.SilentAimPrediction = v end, "SilentAimPrediction"})
Tabs.SilentAim:AddSlider({"Hit chance", 0, 100, 1, Controller.Settings.SilentAimHitChance, function(v) Controller.Settings.SilentAimHitChance = v end, "SilentAimHitChance", "%"})

Tabs.SilentAim:AddSection("Targeting")
Tabs.SilentAim:AddDropdown({"Target type", {"Players and Drones", "Players", "Drones"}, {"Players and Drones"}, function(v) Controller.Settings.SilentAimTargetType = v Controller.SilentTarget = nil Controller.SilentTargetKind = nil end, "SilentAimTargetType", false})
Tabs.SilentAim:AddDropdown({"Player target part", {"Head", "UpperTorso", "Torso", "HumanoidRootPart", "Random"}, {"Head"}, function(v) Controller.Settings.SilentAimTargetPart = v end, "SilentAimTargetPart", false})
Tabs.SilentAim:AddSlider({"FOV radius", 20, 500, 1, Controller.Settings.SilentAimFOV, function(v) Controller.Settings.SilentAimFOV = v end, "SilentAimFOV", " px"})
Tabs.SilentAim:AddSlider({"Max distance", 100, 5000, 1, Controller.Settings.SilentAimMaxDistance, function(v) Controller.Settings.SilentAimMaxDistance = v end, "SilentAimMaxDistance", " studs"})

Tabs.SilentAim:AddSection("FOV")
Tabs.SilentAim:AddToggle({"Show FOV", Controller.Settings.SilentAimShowFOV, function(v) Controller.Settings.SilentAimShowFOV = v end, "SilentAimShowFOV"})
Tabs.SilentAim:AddSlider({"FOV thickness", 1, 4, 1, Controller.Settings.SilentAimFOVThickness, function(v) Controller.Settings.SilentAimFOVThickness = v end, "SilentAimFOVThickness"})
Tabs.SilentAim:AddSlider({"FOV opacity", 10, 100, 1, Controller.Settings.SilentAimFOVOpacity*100, function(v) Controller.Settings.SilentAimFOVOpacity = v/100 end, "SilentAimFOVOpacity", "%"})
Tabs.SilentAim:AddTextBox({"FOV color", color3ToHex(Controller.Settings.SilentAimFOVColor), true, function(t) local c = hexToColor3(t) Controller.Settings.SilentAimFOVColor = c end, "FOV color"})
Tabs.SilentAim:AddTextBox({"Locked FOV color", color3ToHex(Controller.Settings.SilentAimLockedFOVColor), true, function(t) local c = hexToColor3(t) Controller.Settings.SilentAimLockedFOVColor = c end, "Locked FOV color"})

Tabs.ESP:AddSection("Players")
Tabs.ESP:AddToggle({"Player ESP", Controller.Settings.PlayerEnabled, function(v) Controller.Settings.PlayerEnabled = v end, "PlayerEnabled"})
Tabs.ESP:AddToggle({"Boxes", Controller.Settings.PlayerBoxes, function(v) Controller.Settings.PlayerBoxes = v end, "PlayerBoxes"})
Tabs.ESP:AddToggle({"Names", Controller.Settings.PlayerNames, function(v) Controller.Settings.PlayerNames = v end, "PlayerNames"})
Tabs.ESP:AddToggle({"Distance", Controller.Settings.PlayerDistance, function(v) Controller.Settings.PlayerDistance = v end, "PlayerDistance"})
Tabs.ESP:AddToggle({"Tracers", Controller.Settings.PlayerTracers, function(v) Controller.Settings.PlayerTracers = v end, "PlayerTracers"})
Tabs.ESP:AddToggle({"Show teammates", Controller.Settings.ShowTeammates, function(v) Controller.Settings.ShowTeammates = v end, "ShowTeammates"})
Tabs.ESP:AddSlider({"Max distance", 100, 5000, 1, Controller.Settings.PlayerMaxDistance, function(v) Controller.Settings.PlayerMaxDistance = v end, "PlayerMaxDistance", " studs"})

Tabs.ESP:AddSection("Drones")
Tabs.ESP:AddToggle({"Drone ESP", Controller.Settings.DroneEnabled, function(v) Controller.Settings.DroneEnabled = v end, "DroneEnabled"})
Tabs.ESP:AddToggle({"Boxes", Controller.Settings.DroneBoxes, function(v) Controller.Settings.DroneBoxes = v end, "DroneBoxes"})
Tabs.ESP:AddToggle({"Owner names", Controller.Settings.DroneNames, function(v) Controller.Settings.DroneNames = v end, "DroneNames"})
Tabs.ESP:AddToggle({"Team labels", Controller.Settings.DroneTeams, function(v) Controller.Settings.DroneTeams = v end, "DroneTeams"})
Tabs.ESP:AddToggle({"Distance", Controller.Settings.DroneDistance, function(v) Controller.Settings.DroneDistance = v end, "DroneDistance"})
Tabs.ESP:AddToggle({"Tracers", Controller.Settings.DroneTracers, function(v) Controller.Settings.DroneTracers = v end, "DroneTracers"})
Tabs.ESP:AddToggle({"Rear drone arrows", Controller.Settings.DroneRearArrows, function(v) Controller.Settings.DroneRearArrows = v end, "DroneRearArrows"})
Tabs.ESP:AddToggle({"Team check", Controller.Settings.DroneTeamCheck, function(v) Controller.Settings.DroneTeamCheck = v end, "DroneTeamCheck"})
Tabs.ESP:AddToggle({"Show own drone", Controller.Settings.ShowOwnDrone, function(v) Controller.Settings.ShowOwnDrone = v end, "ShowOwnDrone"})
Tabs.ESP:AddSlider({"Max distance", 100, 5000, 1, Controller.Settings.DroneMaxDistance, function(v) Controller.Settings.DroneMaxDistance = v end, "DroneMaxDistance", " studs"})
Tabs.ESP:AddButton({"Rescan drones", Desc = "Manually rescan for drones", Callback = discoverDrones})

Tabs.ESP:AddSection("Visuals")
Tabs.ESP:AddTextBox({"Enemy color", color3ToHex(Controller.Settings.EnemyColor), true, function(t) local c = hexToColor3(t) Controller.Settings.EnemyColor = c end, "Enemy color"})
Tabs.ESP:AddTextBox({"Teammate color", color3ToHex(Controller.Settings.TeammateColor), true, function(t) local c = hexToColor3(t) Controller.Settings.TeammateColor = c end, "Teammate color"})
Tabs.ESP:AddTextBox({"Friendly drone color", color3ToHex(Controller.Settings.FriendlyDroneColor), true, function(t) local c = hexToColor3(t) Controller.Settings.FriendlyDroneColor = c end, "Friendly drone color"})
Tabs.ESP:AddTextBox({"Enemy drone color", color3ToHex(Controller.Settings.EnemyDroneColor), true, function(t) local c = hexToColor3(t) Controller.Settings.EnemyDroneColor = c end, "Enemy drone color"})
Tabs.ESP:AddTextBox({"Unknown drone color", color3ToHex(Controller.Settings.UnknownDroneColor), true, function(t) local c = hexToColor3(t) Controller.Settings.UnknownDroneColor = c end, "Unknown drone color"})
Tabs.ESP:AddSlider({"Box thickness", 1, 4, 1, Controller.Settings.BoxThickness, function(v) Controller.Settings.BoxThickness = v end, "BoxThickness"})
Tabs.ESP:AddSlider({"Text size", 10, 24, 1, Controller.Settings.TextSize, function(v) Controller.Settings.TextSize = v end, "TextSize"})
Tabs.ESP:AddSlider({"Opacity", 20, 100, 1, Controller.Settings.Opacity*100, function(v) Controller.Settings.Opacity = v/100 end, "Opacity", "%"})
Tabs.ESP:AddSlider({"Rear arrow radius", 30, 180, 1, Controller.Settings.RearArrowRadius, function(v) Controller.Settings.RearArrowRadius = v end, "RearArrowRadius", " px"})
Tabs.ESP:AddSlider({"Rear arrow size", 6, 24, 1, Controller.Settings.RearArrowSize, function(v) Controller.Settings.RearArrowSize = v end, "RearArrowSize", " px"})
...
if not Controller.Settings.SilentAimEnabled then
